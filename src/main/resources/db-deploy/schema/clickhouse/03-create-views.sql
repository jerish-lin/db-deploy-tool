-- ClickHouse Views for SchemaFlow (Refactored)
...
SELECT
    cs.script_name,
    cs.script_checksum,
    countIf(ca.execution_status = 'ROLLED_BACK') AS rollback_count,
    maxIf(ca.execution_time, ca.execution_status = 'ROLLED_BACK') AS last_rollback_time
FROM schemaflow_changelog_script cs
LEFT JOIN schemaflow_changelog_audit ca ON cs.id = ca.script_id
