...
public class AuditDao {
    ...
    public void recordScriptExecution(ChangeLogEntry entry) throws SQLException {
        String sql = """
            INSERT INTO db_change_log (
                ...
                rollback_script_content, tag_name, context, labels, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """;
        ...
        try (Connection connection = connectionManager.getConnection();
            ...
            statement.setString(12, entry.getContext());
            statement.setString(13, entry.getLabels());
            
            if (isSQLite) {
                statement.setString(14, entry.getCreatedAt().toString());
                statement.setString(15, entry.getUpdatedAt().toString());
            } else {
                statement.setTimestamp(14, Timestamp.valueOf(entry.getCreatedAt()));
                statement.setTimestamp(15, Timestamp.valueOf(entry.getUpdatedAt()));
            }
    ...
    private ChangeLogEntry mapResultSetToChangeLogEntry(ResultSet resultSet) throws SQLException {
        ...
        entry.setContext(resultSet.getString("context"));
        entry.setLabels(resultSet.getString("labels"));
