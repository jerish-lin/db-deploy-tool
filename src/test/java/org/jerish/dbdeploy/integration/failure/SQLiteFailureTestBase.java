...
public abstract class SQLiteFailureTestBase extends SQLiteDeployTestBase {
    ...
    protected void verifySuccessfulDeploymentV1_0_1() {
        ...
        String[] expectedTables = {"users", "orders", "schemaflow_change_log", "schemaflow_deploy_lock"};
        ...
        for (String scriptId : v1_0_1_Scripts) {
            Integer scriptCount = dbDeployJdbcTemplate.queryForObject(
                    "SELECT COUNT(*) FROM schemaflow_change_log WHERE script_name=? AND execution_status='SUCCESS'",
    ...
    protected void verifyFailureState(String failedSql) {
        ...
        Integer firstScriptCount = dbDeployJdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM schemaflow_change_log WHERE script_name='feature-12349-add-user-email-index' AND execution_status='SUCCESS'",
                Integer.class);
        ...
        Integer failedScriptCount = dbDeployJdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM schemaflow_change_log WHERE script_name=? AND execution_status='FAILED'",
        ...
        Integer thirdScriptCount = dbDeployJdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM schemaflow_change_log WHERE script_name='feature-12351-add-order-status-index'",
                Integer.class);
    ...
    protected void verifyRollbackAfterFailure() {
        ...
        Integer rolledBackCount = dbDeployJdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM schemaflow_change_log t1 WHERE t1.script_name='feature-12349-add-user-email-index' AND t1.id = (SELECT MAX(t2.id) FROM schemaflow_change_log t2 WHERE t2.script_name='feature-12349-add-user-email-index') AND t1.execution_status='ROLLED_BACK'",
                Integer.class);
        ...
        Integer failedScriptRolledBackCount = dbDeployJdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM schemaflow_change_log t1 WHERE t1.script_name='feature-12350-create-user-orders-view' AND t1.id = (SELECT MAX(t2.id) FROM schemaflow_change_log t2 WHERE t2.script_name='feature-12350-create-user-orders-view') AND t1.execution_status='ROLLED_BACK'",
                Integer.class);
