...
@DisplayName("Unit tests for SQLiteSchemaInitializationStrategy")
public class SQLiteSchemaInitializationStrategyTest {

    @Mock
    private JdbcTemplate jdbcTemplate;

    private SQLiteSchemaInitializationStrategy strategy;
    ...
    void testGetSchemaFilesBasePath() {
        assertEquals("db-deploy/schema/sqlite", strategy.getSchemaFilesBasePath());
    }
    ...
    void testIsSchemaInitialized_TableExists() {
        ...
        verify(jdbcTemplate).queryForObject(
                eq("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='schemaflow_changelog_script'"),
                eq(Integer.class)
        );
    }
    ...
    @Test
    @DisplayName("Test isSchemaInitialized uses SQLite-specific query")
    void testIsSchemaInitialized_UsesSQLiteQuery() {
        when(jdbcTemplate.queryForObject(anyString(), eq(Integer.class)))
                .thenReturn(1);
    ...
    @Test
    @DisplayName("Test strategy is correctly typed as SQLite")
    void testStrategyType() {
        ...
        assertTrue(strategy instanceof SQLiteSchemaInitializationStrategy);
    }
}
