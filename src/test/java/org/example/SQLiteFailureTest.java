...
public class SQLiteFailureTest {
    ...
    private static final String DB_FILE = "testdb-failure.sqlite";
    ...
    private void verifyFailureState() throws SQLException {
        try (Connection connection = connectionManager.getConnection()) {
            ...
            try (PreparedStatement stmt = connection.prepareStatement(
                    "SELECT COUNT(*) FROM db_change_log WHERE script_id='add-user-email-index' AND execution_status='SUCCESS'")) {
                try (ResultSet rs = stmt.executeQuery()) {
                    assertTrue(rs.next() && rs.getInt(1) == 1, 
            ...
            try (PreparedStatement stmt = connection.prepareStatement(
                    "SELECT COUNT(*) FROM db_change_log WHERE script_id='create-user-orders-view' AND execution_status='FAILED'")) {
                try (ResultSet rs = stmt.executeQuery()) {
                    assertTrue(rs.next() && rs.getInt(1) == 1, 
            ...
            try (PreparedStatement stmt = connection.prepareStatement(
                    "SELECT COUNT(*) FROM db_change_log WHERE script_id='add-order-status-index'")) {
                try (ResultSet rs = stmt.executeQuery()) {
    ...
    private void verifyRollbackAfterFailure() throws SQLException {
        try (Connection connection = connectionManager.getConnection()) {
            ...
            try (PreparedStatement stmt = connection.prepareStatement(
                    "SELECT COUNT(*) FROM db_change_log WHERE script_id='add-user-email-index' AND execution_status='ROLLED_BACK'")) {
                try (ResultSet rs = stmt.executeQuery()) {
                    assertTrue(rs.next() && rs.getInt(1) == 1, 
                        "add-user-email-index script should be marked as ROLLED_BACK");
                }
            }
